...
class DoomsdayOptionStrategy:
    def __init__(self, trading_config, api_config):
        ...
        # 添加 VIX 到订阅列表
        self.symbols = trading_config.get('symbols', ["IONQ.US","TSLL.US","NVDA.US","AAPL.US"])
    ...
    async def run(self):
        ...
        try:
            while True:
                ...
                # 检查是否接近收盘时间 (收盘前15分钟)
                if self.is_near_market_close(current_time):
                    await self.position_manager.close_all_positions_before_market_close()
    ...
    def is_near_market_close(self, current_time: datetime) -> bool:
        """检查是否接近收盘时间"""
        # 美股收盘时间为16:00 EST
        market_close = current_time.replace(hour=16, minute=0, second=0, microsecond=0)
        time_to_close = (market_close - current_time).total_seconds() / 60  # 转换为分钟
        
        # 收盘前15分钟开始执行收盘平仓
        return 0 <= time_to_close <= 15
