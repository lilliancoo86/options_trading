...
class DoomsdayOptionStrategy:
    ...
    async def select_option_contract(self, symbol: str) -> Optional[Dict[str, Any]]:
        """
        选择期权合约
        Params:
            symbol: 标的代码
        ...
        try:
            ...
            if not quote:
                self.logger.error(f"无法获取 {symbol} 的行情数据")
                return None

            # 获取期权链数据
            current_price = float(quote['last_done'])
            expiry_range = (
                datetime.now(self.tz) + timedelta(days=self.strategy_params['min_days_to_expiry']),
                datetime.now(self.tz) + timedelta(days=self.strategy_params['max_days_to_expiry'])
            )

            # 根据策略参数筛选合适的合约
            target_delta = self.strategy_params['target_delta']
            min_volume = self.strategy_params['min_volume']
            max_spread = self.strategy_params['max_bid_ask_spread']
            self.logger.error(f"target_delta {target_delta} min_volume {min_volume} max_spread {max_spread}")

            # 返回筛选后的最佳合约
            return {
                'symbol': f"{symbol}250117C00150000.US",  # 示例格式
                'side': OrderSide.Buy,
                'expiry': expiry_range[1],
